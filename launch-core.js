(function(root) {
  ...
  function assets(config, value) {
    ...
    const goal = '$' + m.goalAmount.toLocaleString('en-US');
    const profile = `${m.displayName} (@${m.username})\n${m.bio}\nGoal: ${goal} — ${m.goalLabel}`;
    const launch = `${m.displayName}: ${goal} goal — ${m.goalLabel}. ${ecosystemUrl}` + (donationUrl ? ` Voluntary support: ${donationUrl} · Chime: $wholedonuts (official app)` : '');
    ...
    return {donationUrl, ecosystemUrl, launch, request, outreach, profile, goal,
  ...
  function donorPage(config, value) {
    ...
    return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escape(config.movement.displayName)} · Whole Donuts +U</title><style>body{font:18px/1.6 system-ui;max-width:720px;margin:4rem auto;padding:1.5rem;background:#f5f0e6;color:#15130f}a{color:#245238}nav{display:flex;gap:1rem;flex-wrap:wrap}.support{display:inline-block;padding:1rem;background:#15130f;color:white;border-radius:.5rem}</style><main><nav><a href="${escape(a.ecosystemUrl)}">+U home</a><a href="${escape(a.ecosystemUrl)}#tnc">TNC</a><a href="${escape(a.ecosystemUrl)}#awd">AWD</a></nav><h1>${escape(config.movement.displayName)}</h1><p>@${escape(config.movement.username)}</p><p>${escape(config.movement.bio)}</p><h2>${escape(a.goal)} goal</h2><p>${escape(config.movement.goalLabel)}</p><p>A fundraising goal, not a statement of funds received. Support Whole Donuts LLC through the options below.</p><p>Whole Donuts LLC is not a nonprofit. Support is optional, separate from store purchases, and not represented as a charitable or tax-deductible donation. No charitable tax receipt, investment return, or automatic reward is offered.</p><p>Review the recipient and terms on Cash App before paying. For Chime, open the official Chime app and enter $wholedonuts. No Chime payment link is generated. This page does not process or schedule payments.</p><a class="support" href="${escape(a.donationUrl)}" rel="noopener noreferrer" target="_blank">Review and support on Cash App ↗</a><p>${escape(a.donationUrl)}</p><p><a href="mailto:thewholedonuts@gmail.com">Contact Whole Donuts LLC</a></p></main></html>`;
  }
